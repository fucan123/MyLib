...
int DesEncrypt(char* out, const char* key, const char* text, int text_length)
{
	SetKey(key);
...
int DesDecrypt(char* out, const char* key, const char* text, int text_length, bool hex)
{
	SetKey(key);
	char newText[8];
	int step = hex ? 16 : 8;
...
void Des_Run(char Out[16], const char* In, int InLength, bool decode)
{
	char text[8];
	...
	ByteToBit(Mingwen, text, 64); //明文转2进制
